...
@implementation YNMoveTableView
...
- (void)yn_addGesture
{
    _gesture = [[UILongPressGestureRecognizer alloc] initWithTarget:self action:@selector(yn_processGesture:)];
    _gesture.minimumPressDuration = _gestureMinimumPressDuration;
    [self addGestureRecognizer:_gesture];
}

- (void)yn_processGesture:(UILongPressGestureRecognizer *)gesture
{
    switch (gesture.state) {
        case UIGestureRecognizerStateBegan:
        {
            [self yn_gestureBegan:gesture];
        }
            break;
        case UIGestureRecognizerStateChanged:
        {
            if (!_canEdgeScroll) {
                [self yn_gestureChanged:gesture];
            }
        }
            break;
        ...
        {
            [self yn_gestureEndedOrCancelled:gesture];
        }
            break;
        default:
            break;
...
- (void)yn_gestureBegan:(UILongPressGestureRecognizer *)gesture
{
    ...
    _tempView = [self yn_snapshotViewWithInputView:cell];
...
- (void)yn_gestureChanged:(UILongPressGestureRecognizer *)gesture
{
    CGPoint point = [gesture locationInView:gesture.view];
    ...
    if (currentIndexPath && ![_selectedIndexPath isEqual:currentIndexPath]) {
        //交换数据源和cell
        [self yn_updateDataSourceAndCellFromIndexPath:_selectedIndexPath toIndexPath:currentIndexPath];
...
- (void)yn_gestureEndedOrCancelled:(UILongPressGestureRecognizer *)gesture
{
    if (_canEdgeScroll) {
        [self yn_stopEdgeScroll];
    }
    //返回交换后的数据源
...
- (UIView *)yn_snapshotViewWithInputView:(UIView *)inputView
{
...
- (void)yn_updateDataSourceAndCellFromIndexPath:(NSIndexPath *)fromIndexPath toIndexPath:(NSIndexPath *)toIndexPath
{
...
- (void)yn_startEdgeScroll
{
    _edgeScrollTimer = [CADisplayLink displayLinkWithTarget:self selector:@selector(yn_processEdgeScroll)];
    [_edgeScrollTimer addToRunLoop:[NSRunLoop mainRunLoop] forMode:NSRunLoopCommonModes];
}

- (void)yn_processEdgeScroll
{
    [self yn_gestureChanged:_gesture];
